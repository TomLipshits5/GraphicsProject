...
Renderer::Renderer(float angle, float relationWH, float near, float far)
{
    ...
    currentViewport = 0;
    isPicked = false;
    cameraPosition = Eigen::Vector3f(0, 0, -10);
}
...
void Renderer::reformatCameras(int cameraIndx, float relationWH){
...
IGL_INLINE void Renderer::Init(igl::opengl::glfw::Viewer* scene, std::list<int>xViewport, std::list<int>yViewport,int pickingBits,igl::opengl::glfw::imgui::ImGuiMenu *_menu)
{
    ...
    if (menu)
    {
        menu->callback_draw_viewer_menu = [&]()
        {
            ...
            auto temp = Eigen::Vector4i(0,0,0,0);
            menu->draw_viewer_menu(scn,cameras,temp, drawInfos);
        };
    }
}

void Renderer::resetCameraParams(int cameraIndx) {
    cameras[cameraIndx]->MyTranslate(Eigen::Vector3d(-cameraPosition.x(), -cameraPosition.y(), -cameraPosition.z()), 1);//MakeTransNoScale was here
    cameras[cameraIndx]->MyRotate(Eigen::Vector3d(1, 0, 0),-cameraXRotation);
    cameras[cameraIndx]->MyRotate(Eigen::Vector3d(0, 1, 0),-cameraYRotation);
    cameraPosition = Eigen::Vector3f(0, 0, 0);
    cameraYRotation=0;
    cameraXRotation=0;
}
