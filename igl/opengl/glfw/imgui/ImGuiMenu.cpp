...
#include "../../../../tutorial/Project/MenueWrapper.h"
                ...
                IGL_INLINE void ImGuiMenu::draw_viewer_menu(igl::opengl::glfw::Viewer *viewer, std::vector<igl::opengl::Camera *> &camera, Eigen::Vector4i &viewWindow, std::vector<DrawInfo *> drawInfos)
                {
                    ...
                    if (ImGui::CollapsingHeader("Mesh", ImGuiTreeNodeFlags_DefaultOpen))
                    {
                        ...
                        if (ImGui::Button("Load##Mesh", ImVec2((w - p) / 2.f, 0)))
                        {
                            ...
                            ((Project *) viewer)->openDialogLoadMesh();
                    ...
                    if (ImGui::CollapsingHeader("Layers", ImGuiTreeNodeFlags_DefaultOpen))
                    {
                        ...
                        {
                            bool isVisible = (((Project *) viewer)->layers.at(i)->isLayerVisible());
                            if (ImGui::Checkbox((((Project *) viewer)->layers.at(i)->getLayerName()).c_str(),
                                                &isVisible))
                            {
                                MenueWrapper::changeLayer((((Project *) viewer)->layers.at(i)->getLayerName()),
                                                          isVisible, (Project *) viewer);
                            }
                        }
                        ImGui::InputText("", viewer->data()->layer_name);
                        if (ImGui::Button("Add LayerClass"))
                        {
                            MenueWrapper::addLayer(viewer->data()->layer_name, true, (Project *) viewer);
                        }
                    }

                    if (ImGui::CollapsingHeader("Picked Shapes", ImGuiTreeNodeFlags_DefaultOpen))
                    {
                        ...
                        if (ImGui::BeginCombo("##materials combo", current_material_item))
                        {
                            for (size_t n = 0; n < viewer->materials.size(); n++)
                            {
                                ...
                                if (ImGui::Selectable(("material" + (std::to_string(n))).c_str(), is_selected))
                                {
                                    ...
                                    MenueWrapper::chooseMaterial(n, (Project *) viewer);
                                }
                                if (is_selected)
                                {
                        ...
                        for (size_t i = 0; i < scn->pShapes.size(); i++)
                        {
                            ...
                            if (scn->shapeWrappers.at(pickedIndx).isTransparent == false)
                            {
                        ...
                        {
                            MenueWrapper::changeObjectsTransperancy(isAllTransparent, (Project *) viewer);
                        }

                        ImGui::SliderFloat("BlurDist", &scn->blurDist, scn->GetRenderer()->GetNear(0), scn->GetRenderer()->GetFar(0));
                        ...
                        std::string tempLayerSetStr = ((Project *) viewer)->layers.at(
                                viewer->data()->layerSetIndex)->getLayerName();
                        ...
                        if (ImGui::BeginCombo("##layers set combo", current_layer_set_item))
                        {
                            ...
                            {
                                bool is_selected = strcmp(current_layer_set_item,
                                                          ((Project *) viewer)->layers.at(n)->getLayerName().c_str()) == 0;
                                if (ImGui::Selectable(((Project *) viewer)->layers.at(n)->getLayerName().c_str(), is_selected))
                                {
                                    current_layer_set_item = ((Project *) viewer)->layers.at(n)->getLayerName().c_str();
                                    viewer->data()->layerSetIndex = n;
                                    MenueWrapper::setLayer(((Project *) viewer)->layers.at(n)->getLayerName(),
                                                           (Project *) viewer);
                                }
                                if (is_selected)
                                {
                    ...
                    {
                        if (ImGui::RadioButton("Normal", &(viewer->data()->camera_split), 0))
                        {
                            MenueWrapper::changeSplit(MenueWrapper::CameraSplitMode::NORMAL, (Project *) viewer);
                        }
                        if (ImGui::RadioButton("Split by x", &(viewer->data()->camera_split), 1))
                        {
                            MenueWrapper::changeSplit(MenueWrapper::CameraSplitMode::BY_X, (Project *) viewer);
                        }
                        if (ImGui::RadioButton("Split by y", &(viewer->data()->camera_split), 2))
                        {
                            MenueWrapper::changeSplit(MenueWrapper::CameraSplitMode::BY_Y, (Project *) viewer);
                        }
                        ...
                        if (ImGui::BeginCombo("##cameras combo 1", current_item))
                        {
                            for (size_t n = 0; n < viewer->data()->cameras.size(); n++)
                            {
                                ...
                                if (ImGui::Selectable(viewer->data()->cameras.at(n).c_str(), is_selected))
                                {
                                    ...
                                    MenueWrapper::setCamera(0, n, (Project *) viewer);
                        ...
                        if (ImGui::BeginCombo("##cameras combo 2", current_item2))
                        {
                            for (size_t n = 0; n < viewer->data()->cameras.size(); n++)
                            {
                                ...
                                if (ImGui::Selectable(viewer->data()->cameras.at(n).c_str(), is_selected))
                                {
                                    ...
                                    MenueWrapper::setCamera(1, n, (Project *) viewer);
                        ...
                        if (ImGui::Button("Add Camera"))
                        {
                            ...
                            MenueWrapper::addCamera(viewer->data()->camera_name, (Project *) viewer);
                        }
                    }

                    if (ImGui::CollapsingHeader("Animation", ImGuiTreeNodeFlags_DefaultOpen))
                    {
                        if (ImGui::Checkbox("Play", &(viewer->data()->play_is_active)))
                        {
                            viewer->isActive = viewer->data()->play_is_active;
                            MenueWrapper::resetTime(viewer->isActive, (Project *) viewer);
                            if (viewer->isActive)
                                viewer->Activate();
                            else
                                viewer->Deactivate();
                        ...
                        // ImGui::DragFloat("Max time ", &(project->mTime), 0.05f, 0.0f, 100.0f, "%.0f");
                        if (ImGui::SliderFloat("Time", &(project->time), 0, (project)->mTime, "%.1f")) {}

                        // Adding to viewer time param
                        if (ImGui::CollapsingHeader("Motion", ImGuiTreeNodeFlags_DefaultOpen))
                        {
                            ImGui::InputFloat3("Point",project->bizieeP);
                            if (ImGui::Button("Add point"))
                                MenueWrapper::addPoint(project->bizieeP, (Project *) viewer);
                            ImGui::InputFloat2("time", project->animationTimes);
                            if (ImGui::Button("Add movment"))
                                MenueWrapper::addMovment(project->animationTimes, (Project *) viewer);
                            if (ImGui::Button("Remove movment"))
                                MenueWrapper::removeMovment((Project *) viewer);
                        }
